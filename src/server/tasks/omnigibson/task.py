...
class OmniGibson(Task):
    def __init__(
        ...
        **configs,
        ...
        self.vab_source_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "vab_omnigibson_src"
        )
        modified_omnigibson_src = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "modified_omnigibson_src"
        )
        ...
        Container.use_apptainer = container_type.lower() == "apptainer"
        if sif_file_path:
            Container.sif_file_path = sif_file_path
    ...
    async def start_sample(
        ...
        try:
            ...
            return TaskSampleExecutionResult(
                status=SampleStatus.TASK_ERROR, result={"error": error_info}
            )
        finally:
            try:
                container.close()
...
async def main():
    ...
    use_apptainer = True  # 默认使用Apptainer
    task = OmniGibson(
